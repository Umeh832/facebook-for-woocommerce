...
/**
	...
	public function test_class_exists() {
		$this->assertTrue( class_exists( CostOfGoods::class ) );
		$cogs = new CostOfGoods();
		$this->assertInstanceOf( CostOfGoods::class, $cogs );
	}

    public function test_supported_integrations() {
        $supported_integrations = CostOfGoods::get_supported_integrations();
        $actual = array_keys( $supported_integrations );
        $expected = array( 'WooC', 'WPFactory' );

        $this->assertSame(array_diff($expected, $actual), array_diff($actual, $expected));
    }

    public function test_return_false_when_no_provider_is_available() {
        $plugin_mock = $this->createMock( WooCCogsProvider::class );
        $plugin_mock->expects( $this->once() )
			->method( 'is_available' )
			->with( 'WooCCogsProvider.php' )
			->willReturn( false );
        
        
    }

    public function test_return_cogs_when_product_has_cogs() {
        $product = $this->createMock( \WC_Product::class );
        $product->method( 'get_cogs_total_value' )
        ->willReturn( 123 ); 
    }
